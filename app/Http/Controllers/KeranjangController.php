...
class KeranjangController extends Controller
{
    ...
    public function proses_invoice(Request $request, $kode_invoice)
    {
        ...
        $invoice = Invoice::where('kode_invoice', $kode_invoice)->first();
        $invoice->pengiriman_id = $request->get('pengiriman');
    ...
    public function proses_pembayaran(Request $request)
    {
        ...
        return redirect()->route('pesanan')->with('berhasil', 'Berhasil melakukan pembayaran.');
    }

    // Menampilkan halaman pengiriman
    public function pengiriman($id, $kode_invoice)
    {
        $ongkir = Pengiriman::where('id', $id)->first();
        $cek_data = Pengiriman::where('id', $id)->count();
        $invoice = Invoice::where('kode_invoice', $kode_invoice)->first();
        ...
        if ($cek_data > 0) {
            $ongkos = $ongkir->biaya;
            $total = $ongkir->biaya + array_sum($total);
        } else {
            $ongkos = 0;
        ...
        if ($invoiceID->count() > 0) {
            ...
            if ($request->get('jumlah') > 0) {
                ...
                if ($request->get('jumlah') > $produk->stok) {
                    ...
                    $kode_invoice = 'INV'.date('dmY').strtoupper(Str::random(5));
                    $invoice = new Invoice;
